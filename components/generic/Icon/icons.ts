...
const isIconAvailable = (name: string) => names().indexOf(name) !== -1;
const getIconFromLocal = (name: string) => {
  const allIcons = all();
  return isIconAvailable(name) ? allIcons[name] : allIcons["unknown"];
};

export { getIconFromLocal };
